...
function getEventDateFromConfig() {
    const rawDate = String((SiteConfig && SiteConfig.pareja && SiteConfig.pareja.fecha) || '').trim();
    const match = rawDate.match(/^(\d{2})-(\d{2})-(\d{4})$/);
    if (!match) return new Date('2026-07-18T00:00:00').getTime();
...
function applyCalendarConfig() {
    ...
    const endDate = `${year}${month}${String(Number(day) + 1).padStart(2, '0')}`;
    const eventTitle = encodeURIComponent(`Mis Quince ${String(SiteConfig.seo.titulo || '').replace(' | Mis XV 2026', '')}`);
    const details = encodeURIComponent('Celebración de mis XV');

    calendarButton.href = `https://calendar.google.com/calendar/render?action=TEMPLATE&text=${eventTitle}&dates=${startDate}/${endDate}&details=${details}`;
}
